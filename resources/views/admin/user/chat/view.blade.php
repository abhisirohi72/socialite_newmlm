                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="chat-container">
                                <div class="row">
                                    <div class="col-md-4" style="border: 1px solid #CCC;background: white;box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3);">
                                        @foreach($all_users as $key=>$user)
                                        <div class="row" style="border-bottom: 1px solid #CCC;cursor:pointer;" onclick="userClick({{ $user->id }})">
                                            <div class="col-md-12 text-center">
                                                <p class="text-primary email_text row_{{ $user->id }} @if($key==0) active @endif" style="position: relative;top: 10px;">
                                                    {{ $user->email }}
                                                </p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-8">
                                        <div class="chat-box" id="chat-box">
                                            @foreach ($chat_details as $item)
                                                <div class="message @if($item->receiver_id==auth()->id()) received @else sent @endif">
                                                    <p>{{ $item->message }}</p>
                                ...
                                <div class="input-box">
                                    ...
                                    <input type="hidden" name="send_user_id" id="send_user_id" value="{{ $all_users[0]->id }}">
...
<script>
    ...
    function userClick(user_id){
        ...
        axios.get('/admin/show_messages/'+user_id).then(response => {
            $(".email_text").removeClass("active");
            $(".row_"+user_id).addClass("active");
    ...
    function sendMessage() {
        ...
        const send_user_id = document.getElementById("send_user_id").value;
        axios.post('/admin/messages', { message, send_user_id }).then(() => {
            document.getElementById("message").value = "";
            userClick(send_user_id);
        });
    }
